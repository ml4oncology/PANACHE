...
def get_patch_attention_scores(
    patch, model256, scale=1, device256=torch.device("cuda:0")
):
    r"""
...
def concat_4_images(im1, im2, im3, im4):
    print(img1.size, im2.size, img3.size, im4.size)
    canvas = Image.new("RGB", (im1.width + im2.width, im1.height + img3.height))
    ...
    for j in range(6):
        ...
        for i in range(6):
            ...
            canvas = concat_4_images(
                ...
                Image.fromarray(region4k_hm),
                Image.fromarray(region256_hm),
                Image.fromarray(region_hm),
            )
            canvas.save(
