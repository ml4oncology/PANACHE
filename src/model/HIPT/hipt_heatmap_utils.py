...
def create_patch_heatmaps_indiv(
    ...
    if threshold != None:
        for i in range(6):
            ...
            new_score256_2[offset_2:s, offset_2:s] = score256_2[
    ...
    for i in range(6):
        ...
        region256_hm = cv2.addWeighted(
            color_block256, alpha, save_region.copy(), 1 - alpha, 0, save_region.copy()
        )
...
def create_patch_heatmaps_concat(
    ...
    if threshold != None:
        ...
        ).save(os.path.join(output_dir, "%s_256th.png" % (fname)))
    ...
    ).save(os.path.join(output_dir, "%s_256hm.png" % (fname)))
...
def get_region_attention_scores(
    ...
    with torch.no_grad():
        ...
        attention_4096 = model4k.get_last_selfattention(
            features_4096.detach().to(device4k)
        )
...
def create_hierarchical_heatmaps_indiv(
    ...
    region3 = add_margin(
        region.crop((128 * 2, 128 * 2, 4096, 4096)),
        top=0,
        left=0,
        bottom=128 * 2,
        right=128 * 2,
    ...
    region4 = add_margin(
        region.crop((128 * 3, 128 * 3, 4096, 4096)),
        top=0,
        left=0,
        bottom=128 * 4,
        right=128 * 4,
    ...
    b256_1, a256_1, a4k_1 = get_region_attention_scores(
        region, model256, model4k, scale, device256=device256, device4k=device4k
    )
    b256_2, a256_2, a4k_2 = get_region_attention_scores(
        region2, model256, model4k, scale, device256=device256, device4k=device4k
    )
    b256_3, a256_3, a4k_3 = get_region_attention_scores(
        region3, model256, model4k, scale, device256=device256, device4k=device4k
    )
    b256_4, a256_4, a4k_4 = get_region_attention_scores(
        region4, model256, model4k, scale, device256=device256, device4k=device4k
    )
    ...
    for j in range(6):
        ...
        new_score4k_2[offset_2:s, offset_2:s] = score4k_2[
            : (s - offset_2), : (s - offset_2)
        ]
        new_score4k_3 = np.zeros_like(score4k_3)
        new_score4k_3[offset_3:s, offset_3:s] = score4k_3[
            : (s - offset_3), : (s - offset_3)
        ]
        new_score4k_4 = np.zeros_like(score4k_4)
        new_score4k_4[offset_4:s, offset_4:s] = score4k_4[
        ...
        color_block4k = (cmap(score4k) * 255)[:, :, :3].astype(np.uint8)
        region4k_hm = cv2.addWeighted(
            color_block4k, alpha, save_region.copy(), 1 - alpha, 0, save_region.copy()
        )
        Image.fromarray(region4k_hm).save(
            os.path.join(output_dir, "%s_1024[%s].png" % (fname, j))
        )
    ...
    for j in range(6):
        ...
        for i in range(6):
            ...
            Image.fromarray(region_hm).save(
                os.path.join(
                    output_dir, "%s_factorized_4k[%s]_256[%s].png" % (fname, j, i)
                )
            )
...
def concat_4_images(im1, im2, im3, im4):
    canvas = Image.new("RGB", (im1.width + im2.width, im1.height + im3.height))
    canvas.paste(im1, (0, 0))
    canvas.paste(im2, (im1.width, 0))
    canvas.paste(im3, (0, im1.height))
    canvas.paste(im4, (im3.width, im2.height))
    return canvas
...
def create_hierarchical_heatmaps_concat(
    ...
    for j in range(6):
        ...
        # taking out original image underlay
        ...
        for i in range(6):
            ...
            canvas = concat_4_images(
                Image.fromarray(save_region),
                Image.fromarray(color_block4k),
                Image.fromarray(color_block256),
                Image.fromarray(color_block),
            )
            canvas.save(
                os.path.join(output_dir, "%s_4k[%s]_256[%s].png" % (fname, j, i))
            )

    return


def create_hierarchical_heatmaps_concat_select(
    region,
    model256,
    model4k,
    output_dir,
