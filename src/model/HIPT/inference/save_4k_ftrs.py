...
from PIL import Image
import numpy as np
from stain_normalizer import normalizeStaining
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('--jpg_dir', type=str, help='directory with subdirectories containing 4096x4096 jpg tiles')
parser.add_argument('--wsi_dict', type=str, help='dictionary with wsi ids and coordinates of tiles')
parser.add_argument('--path_to_checkpoints', type=str, help='path to DINO pretrained checkpoitns (vit256_small and vit4k_xs)')
parser.add_argument('--output_ftr_dir', type=str, help='output directory for 4k tile features')
args = parser.parse_args()
if not os.path.exists(args.path_to_checkpoints):
    parser.error('Invalid path to pretrain checkpoints')
...
class HIPT_4K(torch.nn.Module):
	...
	"""
	def __init__(self, 
		model256_path: str = f'{args.path_to_checkpoints}/vit256_small_dino.pth',
		model4k_path: str = f'{args.path_to_checkpoints}/vit4k_xs_dino.pth', 
		...
		if torch.cuda.device_count() > 1:
			...
			print('Using 2 gpus')
		self.model256 = get_vit256(pretrained_weights=model256_path).to(self.device256)
		self.model4k = get_vit4k(pretrained_weights=model4k_path).to(self.device4k)
...
if not os.path.exists(args.output_ftr_dir):
    os.makedirs(args.output_ftr_dir)
    
model = HIPT_4K()
model.eval()

wsi_dict = torch.load(args.wsi_dict)
for i, slide_id in tqdm(enumerate(wsi_dict['slide_id']), total=len(wsi_dict['slide_id'])):
    slide_dir = os.path.join(args.jpg_dir, slide_id)
    out_dir = os.path.join(args.output_ftr_dir, slide_id)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    for coord in wsi_dict['grid'][i]:
        ftr_name = f'{slide_id}_{coord[0]}_{coord[1]}.pt'
        if not os.path.exists(os.path.join(out_dir, ftr_name)):
#             print(f'Saving feature: {ftr_name}')
            jpg = f'{slide_id}_{coord[0]}_{coord[1]}.jpg'
            img = np.array(Image.open(os.path.join(slide_dir, jpg)))
            img, _, _ = normalizeStaining(img)
            img = eval_transforms()(img).unsqueeze(dim=0)
            ftr = model(img.to(torch.device('cuda:0')))
            torch.save(ftr, os.path.join(out_dir, ftr_name))
        else:
            print(f'Skipping feature {ftr_name}, already saved')
print('Saving HIPT ftrs completed')
