...
parser.add_argument(
    "--jpg_dir",
    type=str,
    help="directory with subdirectories containing 4096x4096 tiles for HIPT input",
)
parser.add_argument(
    "--lib", type=str, help="library file containing wsi ids and coordinates of tiles"
)
parser.add_argument("--ftr_dir", type=str, help="output directory for tile features")
...
class HIPT_4K(torch.nn.Module):
    ...
    def __init__(
        self,
        model256_path: str = "./pretrain_checkpoints/vit256_small_dino.pth",
        model4k_path: str = "./pretrain_checkpoints/vit4k_xs_dino.pth",
...
for i, slide_id in tqdm(enumerate(lib["slide_id"]), total=len(lib["slide_id"])):
    slide_dir = os.path.join(args.jpg_dir, slide_id)
    out_dir = os.path.join(args.ftr_dir, slide_id)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    for coord in lib["grid"][i]:
